...
MB_index_filter_stuck_window = []; % Filtered list of MB's satisfying conditions
...
%------ 
%Remove stuck MB's by count number in window
for i = 1:size(MB_index_filter_count,2)
    ...
    if (MB_density <= MB_dens_condition_stuck)
        MB_index_filter_stuck_window = [MB_index_filter_stuck_window, MB_index];
    end
end
%------
...
for i = 1:size(MB_index_filter_stuck_window,2)
    MB_index = MB_index_filter_stuck_window(i);
...
%Remove low intensity MB's
for i = 1:size(MB_index_filter_stuck_vel,2)
    MB_index = MB_index_filter_stuck_vel(i);
    
    avg_int = sum(MB_log_copy(MB_index).max_int(:))/(MB_log_copy(MB_index).age(3));
    
    
    % Check density condition
    if (avg_int >= MB_condition_int)
        MB_index_filter_stuck_int = [MB_index_filter_stuck_int, MB_index];
    end
end
%------
...
for i = 1:size(MB_index_filter_stuck_int ,2)
   MB_index = MB_index_filter_stuck_int(i);
...
if kidney_center(1) ~= 0
    for i = 1:size(MB_index_filter_avg ,2)
        ...
        % Check density condition
        if (distance_to_center_start > distance_to_center_end)% && (min(MB_density_list) >= MB_dens_condition_single)
...
open(outputVideo);
mov(1:size(MB_index_filter_single,2))= struct('cdata',[],'colormap',[]);
pause(0.2)
